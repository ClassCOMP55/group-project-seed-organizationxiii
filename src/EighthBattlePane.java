...
public class EighthBattlePane extends GraphicsPane {
    ...
    public void showContent() {
        ...
        h1.heal();
        enemy = new MintSentinel(800, 15, 10, "Mint");
    ...
    // =========================================================================
    /**
     * @param onEnemy  true = Mint is blocking (slides from right); false = Hueman (slides from left)
     */
    private void animateShieldBlock(double cx, double cy, String part, boolean onEnemy) {
        ...
        shieldTimer.addActionListener(e -> {
            ...
            if (fr[0] >= tot) {
                ...
                shield = null;
                if (onEnemy) {
                    setBattleMessage("Blocked! Mint defended " + part + ".");
                    updateHealthBars();
                    checkBattleEnd();
                    if (!battleOver) {
                        playerTurn = false;
                    ...
                    setBattleMessage("Guarded! You blocked Mint's " + part + " attack.");
                    updateHealthBars();
                    checkBattleEnd();
                    if (!battleOver) {
                        playerTurn = true;
    ...
    private void showEnemyTurnIntro() {
        setBattleMessage("Mint is preparing to strike...");
        javax.swing.Timer t=new javax.swing.Timer(750,null); t.setRepeats(false);
        t.addActionListener(e->{t.stop();setBattleMessage("Mint attacks — choose your defence!");openDefenseSelection();});
        t.start();
    }
    // =========================================================================
    ...
    private void showVictoryBanner() {
        ...
        GLabel sub=new GLabel("Mint has been defeated."); sub.setFont(new Font("Georgia",Font.PLAIN,Math.max(12,(int)S(22)))); sub.setColor(TEXT_BRIGHT); sub.setLocation(bx+(bw-sub.getWidth())/2,by+bh*.88); mainScreen.add(sub); contents.add(sub); endOverlayObjects.add(sub);
        spawnGoldParticles(getTargetX(enemyImage), enemyImage.getY()+enemyImage.getHeight()/2);
        setBattleMessage("Mint has fallen. Press CONTINUE.");
