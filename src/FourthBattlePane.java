...
public class FourthBattlePane extends GraphicsPane {
    ...
    private static final Color ENEMY_HP = new Color(200, 150, 255);
    ...
    private void buildSprites() {
        ...
        enemyImage = new GImage("lavender.png");

        huemanImage.scale(0.52);
        enemyImage.scale(0.72);

        double hStoneW = S(170), hStoneH = S(35), hCX = X(340), hSY = Y(700);
        GRect hSb2 = new GRect(hCX - hStoneW / 2 - S(6), hSY + S(8), hStoneW + S(12), hStoneH * 0.55);
        hSb2.setFilled(true);
        ...
        GRect eSb2 = new GRect(eCX - eStoneW / 2 - S(6), eSY + S(8), eStoneW + S(12), eStoneH * 0.55);
        eSb2.setFilled(true);
        eSb2.setFillColor(new Color(65, 40, 85));
        eSb2.setColor(new Color(45, 20, 60));
        addObj(eSb2);

        GRect eSb1 = new GRect(eCX - eStoneW / 2, eSY, eStoneW, eStoneH);
        eSb1.setFilled(true);
        eSb1.setFillColor(new Color(145, 100, 180));
        eSb1.setColor(new Color(90, 60, 130));
        addObj(eSb1);
    ...
    private void buildEnemyHUD() {
        ...
        enemyHudAccent.setFillColor(new Color(200, 150, 255));
        enemyHudAccent.setColor(new Color(200, 150, 255));
        addObj(enemyHudAccent);

        enemyNameLabel = new GLabel(e1.getName().toUpperCase());
        enemyNameLabel.setFont(new Font("Georgia", Font.BOLD, Math.max(12, (int) S(28))));
        enemyNameLabel.setColor(new Color(200, 150, 255));
        ...
        double bx = px + pw * 0.18, by = py + ph * 0.52, bw = pw * 0.68, bh = ph * 0.18;
    ...
    private void buildPlayerHUD() {
        ...
        double bx = px + pw * 0.16, by = py + ph * 0.44, bw = pw * 0.62, bh = ph * 0.14;
        ...
        double sxB = px + pw * 0.16, syB = py + ph * 0.68, sw = pw * 0.62, sh2 = ph * 0.12;
    ...
    private String aiChooseAttack() {
        enemyPhase2 = (double) e1.getHP() / enemyMaxHP < 0.40;
    ...
    private void openAttackSelection() {
        if (!battleOver) buildSelectionOverlay("ATTACK LAVENDER", "Choose where to strike.", "lavender.png");
    }
    ...
    private void resolvePlayerAttack(String chosenTarget) {
        ...
        if (!chosenTarget.equals(def)) {
            ...
            setBattleMessage("Lavender is defending " + def + "...");
    ...
    private void animatePlayerProjectile(String chosenTarget, String enemyDefense) {
        ...
        animTimer.addActionListener(e -> {
            ...
            if (cnt[0] >= steps) {
                ...
                projectile = null;
                animating = false;
                setBattleMessage("Hit! Lavender defended " + enemyDefense + ".");
                explodeThenFlinch(getTargetX(enemyImage), getTargetY(enemyImage, chosenTarget), true, () -> {
                    ...
                    e1.takeDamage(dmg);
                    animateEnemyHPDrain(enemyHealthBack.getWidth() * ((e1.getHP() + dmg) / enemyMaxHP),
                            enemyHealthBack.getWidth() * Math.max(0, (double) e1.getHP() / enemyMaxHP));
                });
            }
        });
        animTimer.start();
    ...
    private void animateEnemyProjectile(String enemyAttack, String chosenDefense) {
        if (animating) return;
        animating = true;
        setBattleMessage("Lavender fires at your " + enemyAttack + "!");
        ...
        animTimer.addActionListener(e -> {
            ...
            if (cnt[0] >= steps) {
                ...
                projectile = null;
                animating = false;
                setBattleMessage("Lavender hit your " + enemyAttack + "!");
                explodeThenFlinch(ex, ey, false, () -> {
                    gainSuper(SUPER_GAIN_DMGTAKEN);
                    int dmg = getOneFifthDamage(huemanMaxHP);
                    h1.takeDamage(dmg);
                    animatePlayerHPDrain(huemanHealthBack.getWidth() * ((h1.getHP() + dmg) / huemanMaxHP),
    ...
    private void showEnemyTurnIntro() {
        setBattleMessage("Lavender is preparing an attack...");
    ...
    private void checkBattleEnd() {
        ...
        if (e1.getHP() <= 0) {
            battleOver = true;
            setBattleMessage("Lavender was defeated!");
        } else if (h1.getHP() <= 0) {
            battleOver = true;
    ...
    private void updateHealthBars() {
        if (enemyHealthBar != null && enemyHealthBack != null) {
            double ratio = Math.max(0, (double) e1.getHP() / enemyMaxHP);
            enemyHealthBar.setSize(enemyHealthBack.getWidth() * ratio, enemyHealthBack.getHeight());
        }

        if (huemanHealthBar != null && huemanHealthBack != null) {
            double ratio = Math.max(0, (double) h1.getHP() / huemanMaxHP);
            huemanHealthBar.setSize(huemanHealthBack.getWidth() * ratio, huemanHealthBack.getHeight());
        }
    ...
    private void updateSuperMeter() {
        if (superBar != null && superBack != null) {
    ...
    public void mouseClicked(MouseEvent e) {
        ...
        if (obj == fightOption || obj == fightHighlight) {
            if (playerTurn && !animating) openAttackSelection();
            return;
        }
